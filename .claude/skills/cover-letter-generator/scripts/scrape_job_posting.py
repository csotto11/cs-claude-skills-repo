...
Usage: python3 scrape_job_posting.py "<URL>"
...
def main():
    if len(sys.argv) < 2:
        print(json.dumps({"error": "Usage: python3 scrape_job_posting.py <URL>"}))
        sys.exit(1)

    url = sys.argv[1]

    # Validate URL scheme to prevent SSRF / protocol smuggling
    parsed = urlparse(url)
